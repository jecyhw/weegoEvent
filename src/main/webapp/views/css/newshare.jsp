									...
									<div class="pic">
                                        ...
                                        <c:choose>
											<c:when test="${event.state.name} == '报名中'">
												...
												<a class="button clearfix" href="share_detail.html?id=${event.id}&thumbnail=${image.thumbnail}&detail=${image.detail}&sign_up=${image.sign_up}&partner=${image.partner}"><!-- 参数 -->
												</a>
											</c:when>
											...
											<c:when test="${event.state.name} == '往期回顾'">
												<img class="btn return" src="../resource/img/share/return.jpg">
											</c:when>
										</c:choose>
									</div>
								</c:forEach>
