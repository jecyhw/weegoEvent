							...
							<div class="content">
								...
								<c:forEach items="${dataEvent.events}" var="event">
									...
									<div class="pic">
										...
										<!-- if判断状态 显示不同状态的报名图片-->
                                        <c:choose>
                                            <c:when test="${event.state.name == '报名中'|| event.state.name == '报名截止'}">
											<a href="share_detail.jsp?id=${event.id}&detail=${image.detail}&sign_up=${image.signUp}&partner=${image.partner}"><!-- 参数 -->
												<div class="btn join">
												</div>
											</a>
                                            </c:when>
                                            <c:when test="${event.state.name == '活动即将开始'}">
